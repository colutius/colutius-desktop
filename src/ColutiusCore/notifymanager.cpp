...
void NotifyManager::rearrange()
{
    ...
    for (i = notifyList.begin(); i != notifyList.end(); ++i)
    {
        ...
        connect(animation, &QPropertyAnimation::finished, this, [animation, this]() { animation->deleteLater(); });
    }
}
...
void NotifyManager::showNext()
{
    ...
    QPoint bottomRignt = desktopRect.bottomRight();
    QPoint pos =
        bottomRignt - QPoint(notify->width() + RIGHT, (HEIGHT + SPACE) * (notifyList.size() + 1) - SPACE + BOTTOM);
